...
class Order extends BaseModel
{
    ...
    public $id, $user, $status, $forename, $surname, $phonenumber, $email, $delivery_address, $zipcode, $postoffice, $totalprice;
    ...
    {
        $query = DB::connection()->prepare('SELECT Order1.id, Order1.status_id, Order1.forename, Order1.surname, Order1.email, Order1.phonenumber, Order1.delivery_address, Order1.zipcode, Order1.postoffice FROM Order1 WHERE Order1.user1_id = :user ORDER BY status_id');
        $query->execute(array(
            'user' => $user_id
        ));
        $rows   = $query->fetchAll();
    ...
    {
        $query = DB::connection()->prepare('SELECT Order1.id, Order1.user1_id, Order1.status_id, Order1.forename, Order1.surname, Order1.email, Order1.phonenumber, Order1.delivery_address, Order1.zipcode, Order1.postoffice FROM Order1 ORDER BY status_id');
        $query->execute();
        $rows   = $query->fetchAll();
        $orders = array();
